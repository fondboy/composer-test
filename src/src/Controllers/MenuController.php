...
class MenuController extends Controller
{
        ...
        return $content
            ...
            ->row(function (Row $row) {
                ...
                $row->column(6, function (Column $column) {
                    ...
                    $form->icon('icon', trans('admin.icon'))->default('fa-bars')->help($this->iconHelp());  // 安分 去除图标必填
    ...
    protected function treeView()
    {
        ...
        return $menuModel::tree(function (Tree $tree) {
            ...
            $tree->branch(function ($branch) {
                $payload = "<i class='fa {$branch['icon']}'></i>&nbsp;<strong>{$branch['title']}{$branch['memo']}</strong>";  // 安分 菜单添加memmo显示
    ...
    public function form()
    {
        ...
        $form->select('parent_id', trans('admin.parent_id'))->options($menuModel::selectOptions());
        $form->text('title', trans('admin.title'))->rules('required');
       // $form->text('memo', trans('admin.memo')); // 安分 去除菜单图标必填
        //$form->icon('icon', trans('admin.icon'))->default('fa-bars')->rules('required')->help($this->iconHelp()); // 安分 去除菜单图标必填
